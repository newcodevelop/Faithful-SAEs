#!/usr/bin/env python3
"""
Fixed-target transport + matched-norm random perturbation controls for
...
Experiment 2: Matched-norm random perturbation control
------------------------------------------------------
For each layer l and example/token position, compute the SAE perturbation

    Delta_l^SAE(x) = SAE_recon_l(x) - h_l(x).

Then sample a random perturbation with the same per-token L2 norm:
...
class SAEAdapter:
    ...
    def reconstruct_tensor(self, act: torch.Tensor, k: int) -> torch.Tensor:
...
def evaluate_batch_fixed_targets(
    ...
    with torch.no_grad():
        ...
        # --------------------------
        # Perturbation 2: matched-norm random controls.
        # --------------------------
...
def summarize_detailed(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Add SAE / random ratio rows by merging the two perturbation summaries.
    sae = summary[summary["perturbation"] == "sae"].copy()
    rnd = summary[summary["perturbation"] == "random_matched_norm"].copy()
    ...
    for _, row in merged.iterrows():
        ratio_rows.append({
            ...
            "horizon": row["horizon"],
            "perturbation": "sae_over_random_ratio",
            "top_k": row["top_k"],
            "mean_kl_bits": row["mean_kl_bits_sae"] / max(row["mean_kl_bits_rand"], 1e-12),
            "std_kl_bits": np.nan,
            "mean_norm_l2": row["mean_norm_l2_sae"] / max(row["mean_norm_l2_rand"], 1e-12) if not pd.isna(row["mean_norm_l2_sae"]) else np.nan,
            "std_norm_l2": np.nan,
            "mean_cosine_distance": row["mean_cosine_distance_sae"] / max(row["mean_cosine_distance_rand"], 1e-12) if not pd.isna(row["mean_cosine_distance_sae"]) else np.nan,
            ...
            "n_measurements": min(row["n_measurements_sae"], row["n_measurements_rand"]),
    ...
    for model_name in summary["Model"].unique():
        ...
        for perturbation, pretty in [
            ...
            for metric_name, (col, label) in metrics.items():
                ...
                if perturbation == "sae_over_random_ratio":
                    cmap = "coolwarm"
...
def plot_horizon_lines_from_summary(summary: pd.DataFrame, output_dir: str):
    ...
    for model_name in summary["Model"].unique():
        sub_model = summary[
            ...
            & (summary["perturbation"].isin(["sae", "random_matched_norm"]))
        ].copy()
        if sub_model.empty:
            continue
